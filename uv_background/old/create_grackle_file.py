...
fileName = 'data/CloudyData_UVB=HM2012_cloudy_primordial_metalsOn.h5'
...
for type in file_gk[root_name].keys():
  ...
  for field in group.keys():
    ...
    if type == 'Primordial':
      ...
      data = data_cloudy_primordial[field]
      
    if type == 'Metals':
      if Metals_on:
        print "  Cloudy data: ", field
        data = data_cloudy_metals[field] - data_cloudy_primordial[field]
        data[data <= 0] = 0
        floor = 1e-30
        data += floor
      # print "  Zero data: ", field
      # data = np.zeros_like(data)
    field_group_br = group_br.create_dataset( field, data=data )
    for attr in field_group.attrs.keys():
      print '   ' + attr
      field_group_br.attrs[attr] = field_group.attrs[attr]
